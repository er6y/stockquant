"""Stockquant Enhanced - Single-file version with inline multi-source providers.

Merges stockquant.py + providers/ into ONE file for WeChat distribution.
All provider classes (EfinanceProvider, SinaProvider, TencentProvider,
JQKAProvider, BaoStockProvider) are inlined along with enrichment functions
and MultiSourceProvider + TTLCache fallback chain.

Architecture:
  Section 0 : Imports & Constants
  Section 1 : Provider Classes (efinance, sina, tencent, jqka, baostock)
  Section 2 : Enrichment Functions (chip, dragon-tiger, fundamentals)
  Section 3 : MultiSourceProvider + TTLCache
  Section 4 : _create_full_provider() factory
  Section 5 : Full stockquant.py (original code, untouched)
  Section 6 : Integration patches (fallback hooks)

Generated by _build_full.py - DO NOT manually edit Section 5.
"""
...
# ============================================================
# Section 1: Provider Classes (inline from providers/)
# ============================================================
...
SECTOR_CACHE_TTL: int = 300  # seconds

# ────────────────────────────────────────────────────────────────
#  Error classification
# ────────────────────────────────────────────────────────────────

class EfinanceError(Exception):
    """Base exception for efinance provider errors."""
    pass

class RemoteDisconnectError(EfinanceError):
    """Remote end closed connection (Eastmoney blocking)."""
    pass

class TimeoutError(EfinanceError):
    """Call exceeded timeout."""
    pass

class RateLimitError(EfinanceError):
    """Rate limiting or anti-bot measures detected."""
    pass

class NotSupportedError(EfinanceError):
    """Operation not supported by this provider."""
    pass

def _classify_error(exc: Exception) -> str:
    """Classify an exception into a known error type string."""
    msg = str(exc).lower()
    if isinstance(exc, (FuturesTimeoutError, TimeoutError)):
        return "timeout"
    if any(k in msg for k in (
        "remote", "disconnect", "connection aborted",
        "connection reset", "connection refused", "max retries",
    )):
        return "remote_disconnect"
    if any(k in msg for k in (
        "rate", "limit", "too many", "429", "block",
        "anti", "captcha",
    )):
...
def _ef_call_with_timeout(
    func: callable,
    *args: Any,
    timeout: int = DEFAULT_TIMEOUT,
    **kwargs: Any,
) -> Any:
    """Call *func* in a thread with a hard timeout.

    efinance's internal requests session has no timeout built in, so
    hung connections can block forever.  This wrapper enforces a ceiling.
    Catches ALL exceptions from the inner function to prevent crashes.
    """
    ...
    try:
        ...
        raise TimeoutError(
            f"efinance call timed out after {timeout}s: {func.__name__}"
        )
    except Exception:
        # Re-raise all other exceptions (retry decorator handles them)
        raise
    finally:
...
def _error(msg: str, error_type: str = "error") -> Dict[str, Any]:
    """Wrap an error result."""
    return {"status": "error", "error": msg, "error_type": error_type}

def _not_supported(feature: str = "") -> Dict[str, Any]:
    """Indicate that a feature is not supported."""
    return {
        "status": "not_supported",
        "error": f"Feature not supported: {feature}" if feature
        else "Not supported",
    }

def _retry(func: callable) -> callable:
    """Decorator: retry with exponential backoff, classify errors.

    On final failure, returns _error() instead of raising, so callers
    always get a safe Dict result.
    """
    ...
    def wrapper(*args: Any, **kwargs: Any) -> Any:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                return _not_supported(str(args[0]) if args else "")
            except Exception as exc:
                last_error = exc
                error_type = _classify_error(exc)
                if error_type == "rate_limit_or_anti_bot":
                    backoff = RETRY_BACKOFF ** (attempt + 1)
                    logger.warning(
                        "Rate-limit detected in %s (attempt %d/%d), "
                        "backing off %.1fs",
                        func.__name__, attempt + 1, MAX_RETRIES, backoff,
                    )
                    time.sleep(backoff)
        ...
        return _error(
            ...
            _classify_error(last_error) if last_error else "unknown",
        )
...
def _normalise_code(code: str) -> str:
    """Normalise a stock code to 6-digit pure-digit form.

    Accepts formats like '000001', 'sh600519', 'sz000001', '600519.XSHG'.
    Returns the 6-digit numeric code without prefix/suffix.
    """
    ...
    # Remove suffix like .XSHG, .XSHE
    if "." in code:
        code = code.split(".")[0]
    # Zero-pad to 6 digits if shorter
    if len(code) < 6:
        code = code.zfill(6)
    return code

def _convert_ef_prefix(code: str) -> str:
    """Convert 6-digit code to efinance prefix format (1. for SH, 0. for SZ)."""
    code = _normalise_code(code)
    first = code[0]
    if first == "6" or first == "9":  # Shanghai
        return f"1.{code}"
    elif first in ("0", "3"):  # Shenzhen
        return f"0.{code}"
    else:  # Beijing or unknown
        return f"0.{code}"
...
class EfinanceProvider:
    """Data provider backed by the efinance library (Eastmoney public API).

    Features
    --------
    * K-line data (daily / weekly / monthly)
    * Real-time quote (single & batch)
    * Capital flow (daily & historical)
    * Sector ranking (by change %)
    * Chip distribution (limited – best-effort via akshare fallback)
    """
    ...
    def __init__(self, timeout: int = DEFAULT_TIMEOUT):
        ...
        self._sector_cache: Optional[Dict[str, Any]] = None
        self._sector_cache_ts: float = 0.0
        logger.info("EfinanceProvider initialised (timeout=%ds)", timeout)
    ...
    def get_kline(
        ...
        logger.debug("Fetching K-line: %s %s→%s klt=%d", code, start, end, klt)
    ...
    def get_realtime_quote(self, code: str) -> Dict[str, Any]:
        ...
        -------
        dict
            {"status": "ok", "data": {"price": ..., "change_pct": ...,
             "change_amount": ..., "volume": ..., "amount": ...,
             "turnover": ..., "amplitude": ..., "high": ..., "low": ...,
             "open": ..., "prev_close": ..., "name": ..., "code": ...}}
        """
        import efinance as ef

        code = _normalise_code(code)
        logger.debug("Fetching real-time quote: %s", code)

        ser = _ef_call_with_timeout(
        ...
        # efinance get_quote_snapshot returns a Series with Chinese labels
        data = {
            "code": str(code),
            "name": str(ser.get("名称", "")),
            "price": _safe_float(ser.get("最新价")),
            "change_pct": _safe_float(ser.get("涨跌幅")),
            "change_amount": _safe_float(ser.get("涨跌额")),
            "volume": _safe_float(ser.get("成交量")),
            "amount": _safe_float(ser.get("成交额")),
            "turnover": _safe_float(ser.get("换手率")),
            "amplitude": _safe_float(ser.get("振幅")),
            "high": _safe_float(ser.get("最高")),
            "low": _safe_float(ser.get("最低")),
            "open": _safe_float(ser.get("今开")),
            "prev_close": _safe_float(ser.get("昨收")),
        }

        # Compute amplitude if missing
        if data["amplitude"] is None and data["high"] and data["low"]:
            if data["prev_close"] and data["prev_close"] != 0:
                data["amplitude"] = round(
                    (data["high"] - data["low"]) / data["prev_close"] * 100, 2
                )
    ...
    @_retry
    def get_realtime_quotes(self, codes: List[str]) -> Dict[str, Any]:
        ...
        -------
        dict
            {"status": "ok", "data": {code: {price, change_pct, ...}, ...}}
        """
        import efinance as ef

        codes = [_normalise_code(c) for c in codes]
        logger.debug("Fetching batch real-time quotes: %d codes", len(codes))

        try:
            df = _ef_call_with_timeout(
                ef.stock.get_latest_quote,
                codes,
                timeout=self._timeout + 10,  # batch can take longer
            )
        except Exception:
            # Fallback: fetch one by one
            logger.warning(
                "Batch get_latest_quote failed, falling back to one-by-one"
            )
            return self._batch_sequential(codes)
        ...
        for _, row in df.iterrows():
            ...
            results[c] = {
                ...
                "prev_close": _safe_float(
                    row.get("prev_close") or row.get("昨日收盘")
                ),
            }
    ...
    def _batch_sequential(
        ...
        """Fallback: fetch quotes one at a time."""
        results: Dict[str, Dict[str, Any]] = {}
        for c in codes:
            try:
                r = self.get_realtime_quote(c)
                if r["status"] == "ok":
                    results[c] = r["data"]
            except Exception as exc:
                logger.warning("Failed to fetch %s: %s", c, exc)
                results[c] = {"code": c, "error": str(exc)}
    ...
    @_retry
    def get_capital_flow(self, code: str) -> Dict[str, Any]:
        """Fetch today's capital flow for a single stock.

        Uses efinance's get_today_bill for intraday flow or
        get_history_bill last row for the latest daily flow.
        ...
        code = _normalise_code(code)
        logger.debug("Fetching capital flow: %s", code)

        # Try get_history_bill first (more reliable for daily data)
        try:
            df = _ef_call_with_timeout(
        ...
        for cn, en in col_map.items():
            if cn in last.index:
                data[en] = _safe_float(last.get(cn))
            else:
                data[en] = None

        # Rename keys back if they weren't mapped
        if "date" not in data:
            data["date"] = str(last.get("日期", ""))

        return _ok(data)

    @_retry
    def get_capital_flow_5d(self, code: str) -> Dict[str, Any]:
        """Fetch the last 5 trading days of capital flow data.

        Returns
        -------
        dict
            {"status": "ok", "data": [
                {date, main_net_inflow, super_large_net, large_net,
                 medium_net, small_net, main_inflow_ratio}, ...
            ]}
        """
        import efinance as ef

        code = _normalise_code(code)
        logger.debug("Fetching 5-day capital flow: %s", code)
        ...
        for _, row in df.iterrows():
            ...
            for key in col_map.values():
                if key in df.columns:
                    rec[key] = _safe_float(row.get(key))
                else:
                    rec[key] = None
            records.append(rec)
    ...
    def get_sector_ranking(self) -> Dict[str, Any]:
        """Fetch industry sector ranking sorted by change_pct (descending).
        ...
        # Check cache
        now = time.time()
        if (self._sector_cache and
                now - self._sector_cache_ts < SECTOR_CACHE_TTL):
            return self._sector_cache  # type: ignore[return-value]

        logger.debug("Fetching sector ranking (cache miss)")
        ...
        try:
            ...
            logger.error("Sector ranking failed: %s", exc)
            return _error(f"Sector ranking failed: {exc}")
        ...
        for _, row in df.iterrows():
            s = {
                ...
                "change_pct": _safe_float(row.get("change_pct")),
                "change_amount": _safe_float(row.get("change_amount")),
                "index_value": _safe_float(row.get("index_value")),
            }
            sectors.append(s)
    ...
    def get_chip_distribution(self, code: str) -> Dict[str, Any]:
        """Fetch chip distribution data (best-effort).

        Note: efinance does not have a direct chip-distribution endpoint.
        This returns a limited best-effort result.  For full chip analysis,
        use the enrichment.chip_distribution module (which falls back to
        akshare).

        Returns
        -------
        dict
            {"status": "not_supported", ...} – always.
        """
        # efinance has no chip distribution endpoint; defer to enrichment
        return _not_supported("chip_distribution (use enrichment module)")

    # ── Fundamental info ────────────────────────────────────────

    @_retry
    def get_base_info(self, code: str) -> Dict[str, Any]:
        """Fetch fundamental info for a stock (PE, PB, ROE, sector, etc.).

        Returns
        -------
        dict
            {"status": "ok", "data": {pe, pb, roe, sector, ...}}
        """
        import efinance as ef

        code = _normalise_code(code)
        logger.debug("Fetching base info: %s", code)
        ...
        data = {
            "code": code,
            "name": str(ser.get("股票名称", "")),
            "pe": _safe_float(ser.get("市盈率(动)") or ser.get("动态市盈率")),
            "pb": _safe_float(ser.get("市净率")),
            "roe": _safe_float(ser.get("ROE")),
            "sector": str(ser.get("所处行业", "")),
            "total_market_cap": _safe_float(ser.get("总市值")),
            "float_market_cap": _safe_float(ser.get("流通市值")),
            "net_profit": _safe_float(ser.get("净利润")),
            "gross_margin": _safe_float(ser.get("毛利率")),
            "net_margin": _safe_float(ser.get("净利率")),
        }
        return _ok(data)
...
_default_provider: Optional[EfinanceProvider] = None

def _get_provider() -> EfinanceProvider:
    """Get or create a default singleton provider."""
    global _default_provider
    if _default_provider is None:
        _default_provider = EfinanceProvider()
...
def get_kline(
    ...
    """Convenience wrapper for EfinanceProvider.get_kline."""
    return _get_provider().get_kline(code, start, end, period)

def get_realtime_quote(code: str) -> Dict[str, Any]:
    """Convenience wrapper for EfinanceProvider.get_realtime_quote."""
    return _get_provider().get_realtime_quote(code)

def get_realtime_quotes(codes: List[str]) -> Dict[str, Any]:
    """Convenience wrapper for EfinanceProvider.get_realtime_quotes."""
    return _get_provider().get_realtime_quotes(codes)

def get_capital_flow(code: str) -> Dict[str, Any]:
    """Convenience wrapper for EfinanceProvider.get_capital_flow."""
    return _get_provider().get_capital_flow(code)

def get_capital_flow_5d(code: str) -> Dict[str, Any]:
    """Convenience wrapper for EfinanceProvider.get_capital_flow_5d."""
    return _get_provider().get_capital_flow_5d(code)

def get_sector_ranking() -> Dict[str, Any]:
    """Convenience wrapper for EfinanceProvider.get_sector_ranking."""
    return _get_provider().get_sector_ranking()

def get_chip_distribution(code: str) -> Dict[str, Any]:
    """Convenience wrapper – always returns not_supported."""
    return _get_provider().get_chip_distribution(code)

def get_base_info(code: str) -> Dict[str, Any]:
    """Convenience wrapper for EfinanceProvider.get_base_info."""
    return _get_provider().get_base_info(code)
...
def _not_supported(feature: str = "") -> Dict[str, Any]:
    return {
        "status": "not_supported",
        "error": f"Feature not supported: {feature}" if feature else "Not supported",
    }
...
def _code_to_sina_sym(code: str) -> str:
    """Convert 6-digit code to sina symbol: sh600519 or sz000001."""
    code = _normalise_code(code)
    first = code[0]
    if first in ("6", "9"):
        return f"sh{code}"
...
class SinaProvider:
    """Data provider backed by Sina Finance public APIs."""
    ...
    PERIOD_TO_SCALE: Dict[str, int] = {
        "1min": 5, "5min": 15, "15min": 30,
        "30min": 60, "60min": 60,
        "daily": 240, "weekly": 1200, "monthly": 2400,
    }
    ...
    def __init__(self, timeout: int = 15):
        ...
        logger.info("SinaProvider initialised (timeout=%ds)", timeout)

    # ── Real-time quote ─────────────────────────────────────────

    def get_realtime_quote(self, code: str) -> Dict[str, Any]:
        """Fetch real-time quote for a single stock via Sina."""
        sym = _code_to_sina_sym(code)
        code_num = _normalise_code(code)
        url = f"{self.SINA_REALTIME_URL}{sym}"
        try:
            resp = self._session.get(url, timeout=self._timeout)
            resp.encoding = "gbk"
            text = resp.text
        except Exception as exc:
            logger.warning("Sina realtime request failed for %s: %s", sym, exc)
            return _error(str(exc))

        if not text or "hq_str_" not in text:
            return _error("Empty or invalid response from Sina")

        try:
            # Extract quoted string
            match = re.search(r'"([^"]*)"', text)
            if not match:
                return _error("Could not parse Sina quote response")
            fields = match.group(1).split(",")

            if len(fields) < 10:
                return _error(f"Insufficient fields in Sina response: {len(fields)}")
            ...
            if data["price"] and data["prev_close"] and data["prev_close"] != 0:
                data["change_pct"] = round(
                    ...
                    / data["prev_close"] * 100, 2,
                )
                data["change_amount"] = round(
                    data["price"] - data["prev_close"], 2,
                )
            else:
                data["change_pct"] = None
                data["change_amount"] = None

            return _ok(data)

        except Exception as exc:
            logger.error("Sina parse error for %s: %s", sym, exc)
            return _error(f"Parse error: {exc}")

    # ── Real-time quotes (batch) ────────────────────────────────

    def get_realtime_quotes(self, codes: List[str]) -> Dict[str, Any]:
        """Batch fetch via comma-separated symbols."""
        syms = [_code_to_sina_sym(c) for c in codes]
        url = f"{self.SINA_REALTIME_URL}{','.join(syms)}"
        code_map = {_code_to_sina_sym(c): _normalise_code(c) for c in codes}
        try:
            resp = self._session.get(url, timeout=self._timeout)
            resp.encoding = "gbk"
            text = resp.text
        except Exception as exc:
            logger.warning("Sina batch request failed: %s", exc)
            return _error(str(exc))

        results: Dict[str, Any] = {}
        for sym in syms:
            code_num = code_map.get(sym, sym)
            try:
                pattern = rf'hq_str_{re.escape(sym)}="([^"]*)"'
                match = re.search(pattern, text)
                if not match:
                    results[code_num] = {"code": code_num, "error": "not found"}
                    continue
                fields = match.group(1).split(",")
                if len(fields) < 10:
                    results[code_num] = {"code": code_num, "error": "insufficient fields"}
                    continue
                ...
                results[code_num] = {"code": code_num, "error": str(exc)}
    ...
    def get_kline(
        ...
        """Fetch K-line data via Sina JSONP API."""
        scale = self.PERIOD_TO_SCALE.get(period)
        if scale is None:
            return _error(f"Unsupported period: {period}")

        sym = _code_to_sina_sym(code)
        code_num = _normalise_code(code)

        # Fetch enough data; Sina returns newest first default
        datalen = 500 if period == "daily" else 200
        ...
        try:
            ...
            logger.warning("Sina K-line request failed for %s: %s", sym, exc)
            return _error(str(exc))
        except json.JSONDecodeError as exc:
            return _error(f"Invalid JSON response: {exc}")
        ...
        try:
            rows = []
            for item in data:
                date_str = str(item.get("day", ""))
                # Filter by date range (normalize all to YYYYMMDD)
                clean_date = date_str.replace("-", "")
                start_clean = start.replace("-", "")
                end_clean = end.replace("-", "")
                if clean_date < start_clean or clean_date > end_clean:
                    continue
                rows.append({
                    "date": date_str,
            ...
            logger.error("Sina K-line parse error for %s: %s", sym, exc)
            return _error(f"Parse error: {exc}")

    # ── Base info ───────────────────────────────────────────────

    def get_base_info(self, code: str) -> Dict[str, Any]:
        """Best-effort: returns limited base info from realtime quote."""
        result = self.get_realtime_quote(code)
        if result["status"] != "ok":
            return result
        q = result["data"]
        return _ok({
            "code": q.get("code", _normalise_code(code)),
            "name": q.get("name", ""),
            "price": q.get("price"),
            "open": q.get("open"),
            "high": q.get("high"),
    ...
    def get_capital_flow(self, code: str) -> Dict[str, Any]:
        return _not_supported("capital_flow (Sina does not provide this via simple API)")

    def get_realtime_quote_raw(self, code: str) -> Dict[str, Any]:
        """Return raw Sina response for debugging."""
        sym = _code_to_sina_sym(code)
        url = f"{self.SINA_REALTIME_URL}{sym}"
        try:
            resp = self._session.get(url, timeout=self._timeout)
            resp.encoding = "gbk"
            return _ok({"raw": resp.text, "symbol": sym})
        except Exception as exc:
            return _error(str(exc))
...
def _get_provider() -> SinaProvider:
    global _provider
...
def _code_to_tencent_sym(code: str) -> str:
    """Convert 6-digit code to tencent symbol: sh600519 or sz000001."""
    code = _normalise_code(code)
    first = code[0]
    if first in ("6", "9"):
        return f"sh{code}"
...
# ────────────────────────────────────────────────────────────────
#  Field index constants (tencent qt A-share, ~ delimited)
# ────────────────────────────────────────────────────────────────

#  0: market (1=A股)    1: name          2: code
#  3: price              4: prev_close    5: open
#  6: volume(手)        7: bid_vol       8: ask_vol
#  9: bid1              10: bid1_size   19: ask1
# 20: ask1_size         31: high         32: low
# 33: amount(万)        38: turnover(%)  43: amplitude(%)
# 45: change_pct(%)     48: datetime
...
def _parse_tencent_quote(sym: str, fields: List[str], code_num: str) -> Dict[str, Any]:
    """Parse a single Tencent quote line into a data dict."""
    if len(fields) <= _IDX_PRICE:
        return {"code": code_num, "error": "insufficient fields"}

    return {
        "code": code_num,
        "name": fields[_IDX_NAME] if len(fields) > _IDX_NAME else "",
        "price": _safe_float(fields[_IDX_PRICE]) if len(fields) > _IDX_PRICE else None,
        "prev_close": _safe_float(fields[_IDX_PREV_CLOSE]) if len(fields) > _IDX_PREV_CLOSE else None,
        "open": _safe_float(fields[_IDX_OPEN]) if len(fields) > _IDX_OPEN else None,
        "high": _safe_float(fields[_IDX_HIGH]) if len(fields) > _IDX_HIGH else None,
        "low": _safe_float(fields[_IDX_LOW]) if len(fields) > _IDX_LOW else None,
        "volume": _safe_float(fields[_IDX_VOLUME]) if len(fields) > _IDX_VOLUME else None,
        "amount": _safe_float(fields[_IDX_AMOUNT]) if len(fields) > _IDX_AMOUNT else None,
        "turnover": _safe_float(fields[_IDX_TURNOVER]) if len(fields) > _IDX_TURNOVER else None,
        "amplitude": _safe_float(fields[_IDX_AMPLITUDE]) if len(fields) > _IDX_AMPLITUDE else None,
        "change_pct": _safe_float(fields[_IDX_CHANGE_PCT]) if len(fields) > _IDX_CHANGE_PCT else None,
        "bid1": _safe_float(fields[_IDX_BID1]) if len(fields) > _IDX_BID1 else None,
        "ask1": _safe_float(fields[_IDX_ASK1]) if len(fields) > _IDX_ASK1 else None,
        "datetime": fields[_IDX_DATETIME] if len(fields) > _IDX_DATETIME else "",
    }
...
class TencentProvider:
    """Data provider backed by Tencent qt (gtimg.cn) public API.

    Strengths: fast real-time quotes, batch-friendly.
    Limitation: no K-line data.
    """
    ...
    def __init__(self, timeout: int = 10):
        ...
        logger.info("TencentProvider initialised (timeout=%ds)", timeout)

    # ── Real-time quote (single) ────────────────────────────────

    def get_realtime_quote(self, code: str) -> Dict[str, Any]:
        """Fetch real-time quote via Tencent qt."""
        sym = _code_to_tencent_sym(code)
        code_num = _normalise_code(code)
        url = f"{self.QT_URL}{sym}"
        try:
            resp = self._session.get(url, timeout=self._timeout)
            resp.encoding = "gbk"
            text = resp.text
        except Exception as exc:
            logger.warning("Tencent request failed for %s: %s", sym, exc)
            return _error(str(exc))

        try:
            # Parse: v_sz002654="1~name~code~price~..."
            match = re.search(r'"([^"]*)"', text)
            if not match:
                return _error("Could not parse Tencent response")
            fields = match.group(1).split("~")
            return _ok(_parse_tencent_quote(sym, fields, code_num))
        except Exception as exc:
            logger.error("Tencent parse error for %s: %s", sym, exc)
            return _error(f"Parse error: {exc}")

    # ── Real-time quotes (batch) ────────────────────────────────

    def get_realtime_quotes(self, codes: List[str]) -> Dict[str, Any]:
        """Batch fetch multiple stocks via comma-separated symbols."""
        syms = [_code_to_tencent_sym(c) for c in codes]
        code_map = {_code_to_tencent_sym(c): _normalise_code(c) for c in codes}
        url = f"{self.QT_URL}{','.join(syms)}"
        try:
            resp = self._session.get(url, timeout=self._timeout + 5)
            resp.encoding = "gbk"
            text = resp.text
        except Exception as exc:
            logger.warning("Tencent batch request failed: %s", exc)
            return _error(str(exc))
        ...
        for sym in syms:
            code_num = code_map.get(sym, sym)
            try:
                # Each line: v_sz002654="data~..."
                pattern = rf'{re.escape("v_" + sym)}="([^"]*)"'
                match = re.search(pattern, text)
                if not match:
                    results[code_num] = {"code": code_num, "error": "not found"}
                    continue
                fields = match.group(1).split("~")
                results[code_num] = _parse_tencent_quote(sym, fields, code_num)
            except Exception as exc:
                results[code_num] = {"code": code_num, "error": str(exc)}
    ...
    def get_kline(
        ...
        """Tencent qt does not provide K-line data. Returns not_supported."""
        return _not_supported("K-line (Tencent qt only provides real-time quotes)")

    # ── Base info ───────────────────────────────────────────────

    def get_base_info(self, code: str) -> Dict[str, Any]:
        """Best-effort: returns info from real-time quote."""
        result = self.get_realtime_quote(code)
        if result["status"] != "ok":
            return result
    ...
    def get_capital_flow(self, code: str) -> Dict[str, Any]:
        return _not_supported("capital_flow (Tencent qt does not provide this)")

# ────────────────────────────────────────────────────────────────
#  Module-level convenience functions
# ────────────────────────────────────────────────────────────────

_provider: Optional[TencentProvider] = None

def _get_provider() -> TencentProvider:
    global _provider
    if _provider is None:
...
def _code_to_jqka_prefix(code: str) -> str:
    """Convert 6-digit code to 10jqka market prefix: sz or sh."""
    code = _normalise_code(code)
    first = code[0]
    if first in ("6", "9"):
        return "sh"
...
# ────────────────────────────────────────────────────────────────
#  K-line parser helpers
# ────────────────────────────────────────────────────────────────

# 10jqka v6/line endpoint uses these type codes:
#   01 = last ~260 days (approx 1 year)
#   04 = full year K-line (year granularity)
# We use 01 for daily/weekly and 04 for longer.

_PERIOD_TO_JQKA_TYPE: Dict[str, str] = {
    "daily": "01",
    "weekly": "01",   # fetch daily then resample is more reliable
    "monthly": "01",
}

def _parse_jqka_line_js(text: str) -> Optional[List[Dict[str, Any]]]:
    """Parse 10jqka v6/line response into list of OHLCV dicts.

    Response is JSONP: quotebridge_v6_line_*_last({json})
    JSON body has "data" field: semicolon-separated day lines.
    Each day line: date,open,high,low,close,volume,amount,...
    """
    try:
        # Try JSONP format first: callback_name({...json...})
        jsonp_match = re.search(r'\((\{.*\})\)\s*$', text, re.DOTALL)
        if jsonp_match:
            ...
            if data_str:
                ...
                for row in data_str.split(";"):
                    ...
                    item = {
                        "date": date_fmt,
                        "open": _safe_float(fields[1]),
                        "high": _safe_float(fields[2]),
                        "low": _safe_float(fields[3]),
                        "close": _safe_float(fields[4]),
                        "volume": _safe_float(fields[5]) if len(fields) > 5 else None,
                        "amount": _safe_float(fields[6]) if len(fields) > 6 else None,
                    }
                    result.append(item)
                return result if result else None

        # Fallback: try JS var assignment format
            ...
            if raw:
                ...
                if len(rows_raw) <= 1:
                    ...
                    if len(parts) >= 7:
                        ...
                        i = 0
                        while i + 6 < len(parts):
                            rows_raw.append(",".join(parts[i:i + 7]))
                            i += 7
                result = []
                for row in rows_raw:
                    fields = row.split(",")
...
class JQKAProvider:
    """Data provider backed by 10jqka (TongHuaShun) public API.

    Strengths: reliable K-line data via JS-var endpoint.
    Limitation: no real-time quote support via simple API.
    """

    name: str = "10jqka"
    priority: int = 3

    JQKA_LINE_URL = "http://d.10jqka.com.cn/v6/line/{prefix}_{code}/{jqka_type}/last.js"
    ...
    def __init__(self, timeout: int = 15):
        ...
        logger.info("JQKAProvider initialised (timeout=%ds)", timeout)
    ...
    def get_kline(
        ...
        """Fetch K-line data via 10jqka v6/line JS-var endpoint."""
        if period not in _PERIOD_TO_JQKA_TYPE:
            return _error(f"Unsupported period: {period}")

        prefix = _code_to_jqka_prefix(code)
        code_num = _normalise_code(code)
        jqka_type = _PERIOD_TO_JQKA_TYPE[period]

        url = self.JQKA_LINE_URL.format(
            prefix=prefix, code=code_num, jqka_type=jqka_type,
        )
        ...
        try:
            ...
            logger.warning("10jqka K-line request failed for %s: %s", code_num, exc)
            return _error(str(exc))

        if not text or len(text) < 50:
            return _error("Empty or invalid response from 10jqka")

        try:
            rows = _parse_jqka_line_js(text)
            if rows is None:
                return _error("Could not parse 10jqka K-line JS response")
            ...
            for row in rows:
                ...
                if clean_date >= start_clean and clean_date <= end_clean:
                    filtered.append(row)
            ...
            logger.error("10jqka K-line parse error for %s: %s", code_num, exc)
            return _error(f"Parse error: {exc}")

    # ── Real-time quote ─────────────────────────────────────────

    def get_realtime_quote(self, code: str) -> Dict[str, Any]:
        """10jqka does not provide simple real-time quote API."""
        return _not_supported("realtime_quote (10jqka simple API does not provide this)")

    def get_realtime_quotes(self, codes: List[str]) -> Dict[str, Any]:
        return _not_supported("realtime_quotes")

    # ── Base info ───────────────────────────────────────────────

    def get_base_info(self, code: str) -> Dict[str, Any]:
        return _not_supported("base_info (10jqka simple API does not provide this)")

    def get_capital_flow(self, code: str) -> Dict[str, Any]:
        return _not_supported("capital_flow")

# ────────────────────────────────────────────────────────────────
#  Module-level convenience functions
# ────────────────────────────────────────────────────────────────

_provider: Optional[JQKAProvider] = None

def _get_provider() -> JQKAProvider:
    global _provider
    if _provider is None:
        _provider = JQKAProvider()
    return _provider
...
def _code_to_baostock(code: str) -> str:
    """Convert 6-digit code to baostock format: sh.600519 or sz.002654."""
    code = _normalise_code(code)
    first = code[0]
    if first in ("6", "9"):
        return f"sh.{code}"
...
# Check baostock availability at import time
_BAOSTOCK_AVAILABLE = False
_BS = None
try:
    import baostock as _BS
    _BAOSTOCK_AVAILABLE = True
    logger.info("baostock library found")
except ImportError:
    logger.warning("baostock library not available; install with: pip install baostock")
...
class BaoStockProvider:
    """Data provider backed by the baostock library.

    Provides: historical daily K-line data.
    Does NOT provide: real-time quotes, capital flow.
    """
    ...
    # Data fields to request from baostock
    BS_FIELDS = "date,open,high,low,close,volume,amount,pctChg"

    def __init__(self):
        self._logged_in = False
        if not _BAOSTOCK_AVAILABLE:
            logger.warning("BaoStockProvider created but baostock is not installed")
            return
        self._login()

    def _login(self):
        """Log in to baostock. Safe to call multiple times."""
        if self._logged_in:
            return
        try:
            lg = _BS.login()
            ...
            logger.error("baostock login exception: %s", exc)

    def _logout(self):
        """Log out from baostock. Safe to call even if not logged in."""
        if not self._logged_in or not _BAOSTOCK_AVAILABLE:
            return
        try:
            _BS.logout()
            self._logged_in = False
            ...
            logger.warning("baostock logout exception: %s", exc)

    def __del__(self):
        # Suppress ALL errors during interpreter shutdown
        # (logging/time modules may already be cleaned up)
        try:
    ...
    def get_kline(
        ...
        """Fetch K-line data via baostock.

        Parameters
        ----------
        code : str
            Stock code (6-digit numeric or with prefix).
        start : str
            Start date in YYYYMMDD or YYYY-MM-DD format.
        end : str
            End date in YYYYMMDD or YYYY-MM-DD format.
        period : str
            Frequency: daily, weekly, monthly, 5min, 15min, 30min, 60min.

        Returns
        -------
        dict
            {"status": "ok", "data": DataFrame} or error dict.
        """
        if not _BAOSTOCK_AVAILABLE:
            return _error("baostock library not installed")

        frequency = self.PERIOD_MAP.get(period)
        ...
        try:
            ...
            if not self._logged_in:
                return _error("baostock login failed")

            bs_code = _code_to_baostock(code)
            code_num = _normalise_code(code)

            # Normalise date format to YYYY-MM-DD for baostock
            start_fmt = self._format_date(start)
            end_fmt = self._format_date(end)

            logger.debug(
                "Fetching baostock K-line: %s %s→%s freq=%s",
                bs_code, start_fmt, end_fmt, frequency,
            )
            ...
            if rs.error_code != "0":
                logger.error(
                    "baostock query error for %s: %s", bs_code, rs.error_msg,
                )
                return _error(f"baostock query error: {rs.error_msg}")
            ...
            # Build DataFrame
            # baostock returns list of lists matching the fields order
            columns = self.BS_FIELDS.split(",")
            ...
            logger.error("baostock K-line error for %s: %s", code, exc)
            return _error(str(exc))

    # ── Real-time quote ─────────────────────────────────────────

    def get_realtime_quote(self, code: str) -> Dict[str, Any]:
        """baostock does not support real-time data."""
        return _not_supported("realtime_quote (baostock is historical data only)")
    ...
    def get_base_info(self, code: str) -> Dict[str, Any]:
        """Try to fetch stock basic info via baostock query_stock_basic."""
        if not _BAOSTOCK_AVAILABLE:
            return _error("baostock library not installed")
        ...
        try:
            ...
            rs = _BS.query_stock_basic(code=bs_code)
            if rs.error_code != "0":
                return _not_supported("base_info (query_stock_basic failed)")
            ...
            logger.error("baostock base_info error for %s: %s", code, exc)
            return _error(str(exc))
...
_provider: Optional[BaoStockProvider] = None

def _get_provider() -> BaoStockProvider:
    global _provider
    if _provider is None:
...
# ============================================================
# Section 2: Enrichment Functions
# ============================================================
...
def _normalise_code(code: str) -> str:
    """Normalise to 6-digit uppercase code."""
    code = str(code).strip().upper()
    for prefix in ("SH", "SZ", "BJ"):
        if code.startswith(prefix):
            code = code[len(prefix):]
...
def _aks_timeout(func: callable, *args: Any, timeout: int = TIMEOUT, **kwargs: Any) -> Any:
    """Call an akshare function with a hard timeout."""
    executor = ThreadPoolExecutor(max_workers=1)
    try:
        future = executor.submit(func, *args, **kwargs)
        return future.result(timeout=timeout)
...
def get_chip_distribution(code: str) -> Dict[str, Any]:
    """Estimate chip distribution for a stock.

    Strategy:
    1. Try akshare stock_individual_fund_flow for 90-day cost estimation
    2. Use efinance get_base_info for ownership structure
    3. Fall back to a reasonable heuristic
    ...
    dict
        {"status": "ok", "data": {
            "avg_cost": float | None,
            "profit_ratio": float | None,   # estimated % of holders in profit
            "concentration_index": float | None,
            "pressure_levels": [{"price": float, "ratio": float}, ...],
        }}
    """
    code = _normalise_code(code)
    logger.debug("get_chip_distribution: %s", code)

    # Attempt 1: akshare fund flow individual (gives cost-related data)
    try:
    ...
    # Try to get recent price data for cost estimation
    try:
        # Use efinance for K-line to estimate avg cost
        import efinance as ef
        ...
        if df_kl is not None and not df_kl.empty:
            close_col = "收盘" if "收盘" in df_kl.columns else "close"
            if close_col in df_kl.columns:
                closes = pd.to_numeric(df_kl[close_col], errors="coerce").dropna()
                if len(closes) > 0:
                    result["avg_cost"] = round(float(closes.mean()), 2)

                    # Estimate profit ratio: % of days price was >= avg cost
                    latest = float(closes.iloc[-1])
                    if latest > 0:
                        # Simple heuristic: profit ratio = current / avg
                        profit = (latest / closes.mean() - 1) * 100
                        result["profit_ratio"] = round(profit, 2)

                    # Pressure levels based on percentile banding
                    if len(closes) >= 30:
                        p90 = float(closes.quantile(0.9))
                        p70 = float(closes.quantile(0.7))
                        p50 = float(closes.quantile(0.5))
                        result["pressure_levels"] = [
                            {"price": round(p90, 2), "ratio": 90.0,
                             "label": "强压力位 (90%分位)"},
                            {"price": round(p70, 2), "ratio": 70.0,
                             "label": "压力位 (70%分位)"},
                            {"price": round(p50, 2), "ratio": 50.0,
                             "label": "中枢位 (50%分位)"},
                        ]
    except Exception as exc:
        logger.warning("Chip estimation via K-line failed: %s", exc)

    # Try efinance for concentration via top10 holders
    try:
        import efinance as ef
        ...
        if top10 is not None and not top10.empty:
            # High concentration = few big holders
            if "持股比例" in top10.columns:
                top10["持股比例"] = pd.to_numeric(
                    top10["持股比例"], errors="coerce"
                )
                top10_sum = top10["持股比例"].dropna().sum()
                result["concentration_index"] = round(float(top10_sum), 2)
    except Exception as exc:
        logger.debug("Top10 holder lookup failed (non-critical): %s", exc)
...
def get_dragon_tiger_data(
    code: str, lookback_days: int = 20
) -> Dict[str, Any]:
    """Check whether a stock appeared on the dragon-tiger list recently.

    Uses akshare stock_lhb_stock_statistic_em().
    ...
    code : str
        Stock code.
    lookback_days : int
        Lookback window in calendar days (default 20).
    ...
    dict
        {"status": "ok", "data": {
            "is_on_list": bool,
            "recent_count": int,
            "records": [{date, buy_amount, sell_amount,
                         net_amount, reason}],
        }}
    """
    code = _normalise_code(code)
    logger.debug("get_dragon_tiger_data: %s, lookback=%d", code, lookback_days)
    ...
    try:
        ...
        logger.error("Dragon tiger data fetch failed: %s", exc)
        return _error(f"akshare stock_lhb_stock_statistic_em failed: {exc}")

    if df is None or df.empty:
        return _ok({"is_on_list": False, "recent_count": 0, "records": []})
    ...
    matches = df[df[code_col].str.contains(code)]

    if matches.empty:
        return _ok({"is_on_list": False, "recent_count": 0, "records": []})

    # Parse dates and filter by lookback
    date_cutoff = datetime.date.today() - datetime.timedelta(days=lookback_days)
    ...
    for _, row in matches.iterrows():
        rec: Dict[str, Any] = {
            ...
            "buy_amount": _safe_float(
                row.get("买入金额") or row.get("买入总计")
            ),
            "sell_amount": _safe_float(
                row.get("卖出金额") or row.get("卖出总计")
            ),
            "net_amount": _safe_float(
                row.get("净买额") or row.get("净买入额")
            ),
            "reason": str(
                row.get("上榜原因", "")
            ),
...
def get_fundamental_alerts(code: str) -> Dict[str, Any]:
    """Collect fundamental alert flags for a stock.

    Combines data from:
    - efinance get_base_info (PE, PB, ROE, sector)
    - akshare stock_financial_analysis_indicator (growth, margins)
    - akshare stock_profit_forecast_em (earnings forecast)
    - akshare stock_history_dividend_detail (dividends)
    ...
    dict
        {"status": "ok", "data": {
            ...
            "earnings": {report_date, revenue, net_profit, operating_cash_flow},
            "forecast": {summary_text, change_range},
            "dividend": {recent_events: [{date, per_share, is_pre_tax}]},
            "alerts": [{severity: 'warning'|'danger', message}],
            "has_bad_news": bool,
        }}
    """
    code = _normalise_code(code)
    logger.debug("get_fundamental_alerts: %s", code)
    ...
    # ── 3a. efinance base info (PE, PB, ROE) ──────────────────
    try:
        import efinance as ef

        base = _aks_timeout(ef.stock.get_base_info, code, timeout=TIMEOUT)
        if base is not None and not base.empty:
            result["growth"]["roe"] = _safe_float(base.get("ROE"))
            result["growth"]["gross_margin"] = _safe_float(base.get("毛利率"))

            # Alert: extremely high PE
            pe = _safe_float(base.get("市盈率(动)") or base.get("动态市盈率"))
            if pe is not None:
                if pe > 200:
                    result["alerts"].append({
                        "severity": "warning",
                        "message": f"市盈率(PE)={pe:.1f}，估值偏高",
                    })
                elif pe <= 0:
                    result["alerts"].append({
                        "severity": "danger",
                        "message": f"市盈率(PE)={pe:.1f}，公司可能亏损",
                    })
                    result["has_bad_news"] = True

            # Alert: PB very low (breakup risk)
            pb = _safe_float(base.get("市净率"))
            if pb is not None and pb < 0.5 and pb > 0:
                result["alerts"].append({
                    "severity": "warning",
                    "message": f"市净率(PB)={pb:.2f}，破净",
                })
    except Exception as exc:
        logger.debug("efinance base_info failed: %s", exc)

    # ── 3b. akshare financial indicators ───────────────────────
    try:
        import akshare as ak
        ...
        if fin is not None and not fin.empty:
            ...
            last = fin.iloc[0] if not fin.empty else None
            if last is not None:
                result["growth"]["revenue_yoy"] = _safe_float(
                    last.get("营业收入同比增长率")
                )
                result["growth"]["net_profit_yoy"] = _safe_float(
                    last.get("归属母公司净利润同比增长率")
                )
                result["growth"]["roe"] = result["growth"]["roe"] or _safe_float(
                    last.get("净资产收益率")
                )
                result["growth"]["gross_margin"] = (
                    result["growth"]["gross_margin"]
                    or _safe_float(last.get("销售毛利率"))
                )
                result["earnings"]["report_date"] = str(
                    last.get("报告期", "")
                )
                result["earnings"]["revenue"] = _safe_float(
                    last.get("营业总收入")
                )
                result["earnings"]["net_profit"] = _safe_float(
                    last.get("归属母公司净利润")
                )

            # Alert: declining revenue
            rev_yoy = result["growth"]["revenue_yoy"]
            np_yoy = result["growth"]["net_profit_yoy"]
            if rev_yoy is not None and rev_yoy < -20:
                result["alerts"].append({
                    "severity": "danger",
                    "message": f"营收同比下滑{abs(rev_yoy):.1f}%，经营恶化",
                })
                result["has_bad_news"] = True
            elif rev_yoy is not None and rev_yoy < 0:
                result["alerts"].append({
                    "severity": "warning",
                    "message": f"营收同比下滑{abs(rev_yoy):.1f}%",
                })

            if np_yoy is not None and np_yoy < -30:
                result["alerts"].append({
                    "severity": "danger",
                    "message": f"净利润同比下滑{abs(np_yoy):.1f}%",
                })
                result["has_bad_news"] = True
    except Exception as exc:
        logger.debug("akshare financial_analysis failed: %s", exc)

    # ── 3c. Earnings forecast ─────────────────────────────────
    try:
        import akshare as ak

        forecast = _aks_timeout(
            ak.stock_profit_forecast_em,
            symbol=code,
            timeout=TIMEOUT,
        )
        if forecast is not None and not forecast.empty:
            last_fc = forecast.iloc[0]
            result["forecast"]["summary_text"] = str(
                last_fc.get("业绩变动摘要", "")
            )
            result["forecast"]["change_range"] = str(
                last_fc.get("业绩变动幅度", "")
            )

            # Check for bad news in forecast
            summary = str(last_fc.get("业绩变动原因", "")).lower()
            if any(kw in summary for kw in (
                ...
                result["alerts"].append({
                    "severity": "danger",
                    "message": f"业绩预告: {result['forecast']['summary_text'][:80]}",
                })
    except Exception as exc:
        logger.debug("Earnings forecast failed: %s", exc)
    ...
    try:
        ...
        if div is not None and not div.empty:
            ...
            for _, row in div.tail(5).iterrows():
                events.append({
                    "date": str(row.get("公告日期", "")),
                    "per_share": _safe_float(
                        row.get("每股转增") or row.get("派息")
                    ),
                    "is_pre_tax": True,
                })
            result["dividend"]["recent_events"] = events
    except Exception as exc:
        logger.debug("Dividend lookup failed: %s", exc)

    # ── 3e. Operating cash flow from akshare ──────────────────
    try:
        import akshare as ak
        ...
        if cf is not None and not cf.empty and "经营活动现金流量净额" in cf.columns:
            result["earnings"]["operating_cash_flow"] = _safe_float(
                cf.iloc[0]["经营活动现金流量净额"]
            )
...
# ────────────────────────────────────────────────────────────────
#  4. News Search (LLM integration point)
# ────────────────────────────────────────────────────────────────

def search_news(query: str, days: int = 3) -> Dict[str, Any]:
    """Placeholder for news search – to be filled via LLM with web_search tool.

    This function returns a structured placeholder.  The calling LLM
    (e.g. stockquant orchestrator) should use the web_search tool to
    populate results and then call this again or merge inline.
    ...
    return _ok([
        {
            "title": f"「{query}」近{days}日新闻占位",
            "source": "LLM orchestration point",
            "date": datetime.date.today().isoformat(),
            "snippet": (
                "This is a placeholder. The calling LLM should use "
                "web_search tool to fetch real news for the query "
                f"'{query}' within the last {days} days."
            ),
            "url": "",
        }
    ])
...
def get_earnings_calendar(code: str) -> Dict[str, Any]:
    """Retrieve upcoming earnings report date, forecast, and surprise history.

    Uses efinance get_all_company_performance or get_all_report_dates.
    ...
    dict
        {"status": "ok", "data": {
            ...
            "next_report_type": str | None,   # 年报/一季报/中报/三季报
            "forecast": str | None,
            "surprise_history": [{date, type, expected, actual, surprise}, ...],
        }}
    """
    code = _normalise_code(code)
    logger.debug("get_earnings_calendar: %s", code)

    result = {
        "next_report_date": None,
        "next_report_type": None,
        "forecast": None,
        "surprise_history": [],
    }
    ...
    try:
        ...
        if dates is not None and not dates.empty:
            # Filter by code
            code_col = next(
                (c for c in ["股票代码", "code", "代码"] if c in dates.columns),
                None,
            )
            if code_col:
                dates[code_col] = dates[code_col].astype(str)
                stock_dates = dates[dates[code_col].str.contains(code)]
                if not stock_dates.empty:
                    today = datetime.date.today()
                    for _, row in stock_dates.iterrows():
                        report_date_str = str(
                            row.get("报告期") or row.get("财报披露日期") or ""
                        )
                        try:
                            rd = datetime.datetime.strptime(
                                report_date_str[:10], "%Y-%m-%d"
                            ).date()
                            if rd >= today:
                                result["next_report_date"] = report_date_str[:10]
                                result["next_report_type"] = str(
                                    row.get("财报类型", "")
                                )
                                break
                        except ValueError:
        ...
        # Try to get performance data for surprise
        perf = _aks_timeout(
        ...
        if perf is not None and not perf.empty:
            code_col = next(
                (c for c in ["股票代码", "code", "代码"] if c in perf.columns),
                None,
            )
            if code_col:
                perf[code_col] = perf[code_col].astype(str)
                stock_perf = perf[perf[code_col].str.contains(code)]
                if not stock_perf.empty:
                    result["forecast"] = str(
                        stock_perf.iloc[0].get("业绩变动摘要", "")
                    )
...
def _days_ago(n: int) -> str:
    """Return date string YYYYMMDD for n days ago."""
...
class TTLCache:
    """A simple in-memory TTL cache for provider results.

    Each key expires after *ttl_seconds*.  Thread-safe via a re-entrant lock.
    """
...
class MultiSourceProvider:
    """Aggregate provider that tries multiple backends in priority order.

    Each method attempts every provider in sequence.  The first successful
    (status == 'ok') result is returned.  If all fail, an error dict is
    returned.

    Attributes
    ----------
    providers : list
        Ordered list of provider instances.  Each must implement the
        methods that will be called through this proxy.
    cache : TTLCache | None
        Optional result cache with configurable TTL.
    """
    ...
    def __init__(
        ...
        """Initialise the fallback provider.

        Parameters
        ----------
        providers : list, optional
            List of provider objects (e.g. EfinanceProvider, SinaProvider, ...).
            If None, create_default_provider() is called.
        cache_ttl : int, optional
            TTL in seconds for the result cache.  None disables caching.
            Default: 300 (5 min) when providers are auto-created.
        """
        if providers is None:
            providers = _auto_discover_providers()
            if cache_ttl is None:
                cache_ttl = 300  # default when auto-creating

        self.providers: List[Any] = list(providers)
        self.cache: Optional[TTLCache] = (
            TTLCache(ttl=cache_ttl) if cache_ttl is not None else None
        )
        self._provider_names: List[str] = [
            type(p).__name__ for p in self.providers
        ]
        logger.info(
            "MultiSourceProvider initialised with %d sources: %s",
            len(self.providers), ", ".join(self._provider_names),
        )
    ...
    def _try_providers(
        ...
        **kwargs: Any,
        ...
        """Try *method_name* on every provider until one succeeds.

        Parameters
        ----------
        method_name : str
            Name of the method to call on each provider.
        cache_key : str, optional
            If provided and caching is enabled, a cached result will be
            returned immediately on cache hit.
        """
        # Check cache first
        ...
        for idx, provider in enumerate(self.providers):
            ...
            if fn is None:
                logger.debug("Provider %s has no method %s", name, method_name)
                errors.append(f"{name}: method not found")
                continue
            ...
            try:
                ...
                if isinstance(result, dict) and result.get("status") == "ok":
                    logger.info(
                        "Method %s: %s succeeded", method_name, name,
                    )
                ...
                # Provider returned an error dict
                err_msg = result.get("error", "unknown") if isinstance(result, dict) else str(result)
                status = result.get("status", "") if isinstance(result, dict) else ""
                errors.append(f"{name}: [{status}] {err_msg}")

                # Don't fallback if feature is explicitly unsupported by all
                if status == "not_supported":
                    logger.debug(
                        "Method %s not supported by %s", method_name, name,
                    )
                    continue

                logger.warning(
                    "Method %s: %s returned error: %s",
                    method_name, name, err_msg,
                )

            except Exception as exc:
                err_str = f"{name}: {type(exc).__name__} - {exc}"
                errors.append(err_str)
                logger.warning(
                    "Method %s: %s raised %s: %s",
                    method_name, name, type(exc).__name__, exc,
                )

        # All providers failed
        summary = "; ".join(errors) if errors else "no providers available"
        logger.error("All providers failed for %s: %s", method_name, summary)
        return _error(f"All providers failed for {method_name}: {summary}")
    ...
    def get_kline(
        ...
        """Fetch K-line with fallback across all providers."""
        return self._try_providers(
            "get_kline",
            code, start=start, end=end, period=period,
            cache_key=f"kline:{code}:{start}:{end}:{period}",
        )
    ...
    def get_realtime_quote(self, code: str) -> Dict[str, Any]:
        ...
        return self._try_providers(
            "get_realtime_quote",
            code,
            cache_key=f"rtq:{code}",
        )

    def get_realtime_quotes(self, codes: List[str]) -> Dict[str, Any]:
        """Fetch batch real-time quotes with fallback.

        Caching disabled for batch calls (key would be too large).
        """
        return self._try_providers("get_realtime_quotes", codes)

    def get_capital_flow(self, code: str) -> Dict[str, Any]:
        """Fetch capital flow with fallback."""
        return self._try_providers(
            "get_capital_flow",
            code,
            cache_key=f"cf:{code}",
    ...
    def get_capital_flow_5d(self, code: str) -> Dict[str, Any]:
        """Fetch 5-day capital flow with fallback."""
        return self._try_providers(
            "get_capital_flow_5d",
            code,
            cache_key=f"cf5d:{code}",
        )

    def get_sector_ranking(self) -> Dict[str, Any]:
        """Fetch sector ranking with fallback."""
        return self._try_providers(
            "get_sector_ranking",
    ...
    def get_chip_distribution(self, code: str) -> Dict[str, Any]:
        """Fetch chip distribution with fallback."""
        return self._try_providers(
    ...
    def get_base_info(self, code: str) -> Dict[str, Any]:
        """Fetch fundamental base info with fallback."""
        return self._try_providers(
            "get_base_info",
            code,
            cache_key=f"base:{code}",
        )

    def clear_cache(self) -> None:
        """Purge the result cache."""
        if self.cache:
            self.cache.clear()
            logger.info("Cache cleared")

    def add_provider(self, provider: Any, position: Optional[int] = None) -> None:
        """Insert a new provider at *position* (default: end)."""
        ...
        if position is None:
            ...
            self._provider_names.insert(position, name)
        logger.info("Added provider %s at position %s", name, position)

    def remove_provider(self, name_or_index: Union[str, int]) -> bool:
        """Remove a provider by class name or index. Returns True if removed."""
        if isinstance(name_or_index, int):
            idx = name_or_index
            if 0 <= idx < len(self.providers):
                removed = self._provider_names.pop(idx)
                self.providers.pop(idx)
                logger.info("Removed provider %s (index %d)", removed, idx)
                return True
            return False

        name = name_or_index
        for idx, pname in enumerate(self._provider_names):
            if pname == name:
                self.providers.pop(idx)
                self._provider_names.pop(idx)
    ...
    def available_providers(self) -> List[str]:
        """Return list of provider class names currently registered."""
...
def _auto_discover_providers() -> List[Any]:
    """Stub — in single-file mode, providers are registered via
    _create_full_provider() instead. This function returns [] so
    create_default_provider() builds the list directly.
    """
    return []
...
def create_default_provider(
    ...
    extra_providers: Optional[List[Any]] = None,
) -> MultiSourceProvider:
    """Create a MultiSourceProvider with all inline provider classes.

    In single-file mode, EfinanceProvider, SinaProvider, TencentProvider,
    JQKAProvider, and BaoStockProvider are registered directly.
    """
    providers: List[Any] = []
    for cls, name in [
        (EfinanceProvider,  "EfinanceProvider"),
        (SinaProvider,      "SinaProvider"),
        (TencentProvider,   "TencentProvider"),
        (JQKAProvider,      "JQKAProvider"),
    ...
    if extra_providers:
        providers = list(extra_providers) + providers

    return MultiSourceProvider(providers=providers, cache_ttl=cache_ttl)

# ────────────────────────────────────────────────────────────────
#  Module-level convenience singleton
# ────────────────────────────────────────────────────────────────

_default_source: Optional[MultiSourceProvider] = None
_lock = threading.Lock()

def get_default_source(cache_ttl: Optional[int] = 300) -> MultiSourceProvider:
    """Get or create the module-level singleton MultiSourceProvider."""
    global _default_source
    if _default_source is None:
        with _lock:
            if _default_source is None:
                _default_source = create_default_provider(cache_ttl=cache_ttl)
    return _default_source

# ============================================================
# Section 4: Single-File Factory
# ============================================================

_full_provider = None

def _create_full_provider():
    """Create MultiSourceProvider with all inline providers (no external imports)."""
    global _full_provider
    providers = []
    try:
        providers.append(EfinanceProvider())
        print("INFO: EfinanceProvider loaded", file=sys.stderr)
    except Exception as e:
        print(f"WARN: EfinanceProvider unavailable: {e}", file=sys.stderr)
    try:
        providers.append(SinaProvider())
        print("INFO: SinaProvider loaded", file=sys.stderr)
    except Exception as e:
        print(f"WARN: SinaProvider unavailable: {e}", file=sys.stderr)
    try:
        providers.append(TencentProvider())
        print("INFO: TencentProvider loaded", file=sys.stderr)
    except Exception as e:
        print(f"WARN: TencentProvider unavailable: {e}", file=sys.stderr)
    try:
        providers.append(JQKAProvider())
        print("INFO: JQKAProvider loaded", file=sys.stderr)
    except Exception as e:
        print(f"WARN: JQKAProvider unavailable: {e}", file=sys.stderr)
    try:
        providers.append(BaoStockProvider())
        print("INFO: BaoStockProvider loaded", file=sys.stderr)
    except Exception as e:
        print(f"WARN: BaoStockProvider unavailable: {e}", file=sys.stderr)

    if not providers:
        print("ERROR: No data providers available!", file=sys.stderr)

    _full_provider = MultiSourceProvider(providers, cache_ttl=300)
    return _full_provider

def _ensure_full_provider():
    """Lazily create the MultiSourceProvider singleton."""
    global _full_provider
    if _full_provider is None:
        _create_full_provider()
    return _full_provider
...
def analyze(queries, include_news=True, days=120, minutes=32,
    ...
    print("""
[🎯 五维权重矩阵 — 最重要的第一条！]
A股本质是零和博弈，分析必须按权重逐层穿透：
  💰 资金面 ⭐⭐⭐⭐⭐ — 主力净流入方向+力度+持续性 = 定方向的核心
  🎭 博弈面 ⭐⭐⭐⭐   — 谁被套/谁会自救/谁在收割/获利盘多厚
  🔥 情绪面 ⭐⭐⭐⭐   — 概念热度+散户追涨意愿+恐慌/贪婪程度
  📊 技术面 ⭐⭐⭐     — 支撑/压力/均线/量价（不单独决策，作为资金面的佐证）
  📋 基本面 ⭐⭐       — 仅做底线过滤（退市风险/暴雷/现金流断裂）

决策优先级链：基本面底线过 → 资金面定向 → 博弈面定时机 → 情绪面定仓位 → 技术面定入场点
任何一步不过，后续免谈。

[博弈面六必答 — 每只股票必须逐条回答！]
1. 谁在里面？机构/北向/游资/散户的筹码分布推断
2. 谁被套了？近期高点买入者的平均成本 ≈？浮亏多少？
3. 谁会自救？主力被套 → 拉高出货；散户被套 → 主力不救
4. 获利盘多厚？从近期低点算，浮盈比例 → 砸盘动机大小
5. 当前谁在买谁在卖？主力净流入 vs 散户净流入 → 谁在接谁的盘
6. 龙虎榜信号（如有）：机构净买/卖？北向净买/卖？
...
def _fetch_kline_with_fallback(code, start="19000101", end="21250101", period="daily"):
    """Fetch K-line via inline provider chain; fall back to stockquant originals.

    The original _fetch_daily_kline 7-source fallback chain is preserved as
    the ultimate fallback when all inline MultiSourceProviders fail.
    """
    try:
        p = _ensure_full_provider()
        result = p.get_kline(
            code,
            start=start.replace("-", ""),
            end=end.replace("-", ""),
        ...
        if result.get("status") == "ok" and len(result.get("data", [])) > 0:
            df = result["data"]
            # Convert DataFrame to stockquant list-of-dict format
            rows = []
            for _, row in df.iterrows():
                rows.append({
                    "date":  str(row.get("date", ""))[:10].replace("-", ""),
        ...
        print(f"WARN: MultiSourceProvider kline failed for {code}: {e}", file=sys.stderr)
    # Ultimate fallback: original 7-source chain
    return _fetch_daily_kline(code, n=30)


def _get_realtime_with_fallback(code):
    """Fetch realtime quote via inline provider chain; fall back to query_quote."""
    try:
        p = _ensure_full_provider()
        result = p.get_realtime_quote(code)
        if result.get("status") == "ok":
            ...
            return {
                ...
                "change":     d.get("change_amount", 0.0),
                "open":       d.get("open", 0.0),
                "high":       d.get("high", 0.0),
                "low":        d.get("low", 0.0),
        ...
        print(f"WARN: MultiSourceProvider realtime failed for {code}: {e}", file=sys.stderr)
